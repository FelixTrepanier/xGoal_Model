...
hyper_grid<-makeTuneControlRandom(maxit = 100L)
...
### First look at the results from XGboost
```{r}
xg_boost.model<-train(xg_boost.learner,task.xG)

xg_boost.pred = getPredictionProbabilities(predict(xg_boost.model, task = task.xG))

head(xg_boost.pred)
mean(xg_boost.pred)

calculateConfusionMatrix(predict(xg_boost.model, task = task.xG))

ggplot()+
  geom_histogram(aes(x=xg_boost.pred))
