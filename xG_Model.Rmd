...
xg_boost.hyper_param<-makeParamSet(makeNumericParam('eta',lower=0.001,upper=0.03),
                                   makeDiscreteParam('booster',values=c('gbtree')),
                                   makeNumericParam('gamma',lower=0.1,upper=0.1),
                                   makeIntegerParam('max_depth',lower=27,upper=27))
...
## Confusion matrix
```{r}
xg_boost.temp<-resample(makeLearner('classif.xgboost',
                                    par.vals = list('eta'=xg_boost.tune$x$eta,
                                                    'booster'=xg_boost.tune$x$booster,
                                                    'gamma'=xg_boost.tune$x$gamma ,
                                                    'max_depth'=xg_boost.tune$x$max_depth)),
                        task = task.xG,
                        resampling = kFold)

calculateConfusionMatrix(xg_boost.temp$pred)
